...
def save_contour_as_ctgr(contours, file_path, z_spacing):
    ...
    for idx, (contour, frame_idx) in enumerate(contours):
        ...
        ET.SubElement(path_point, "pos", x="0", y="0", z=str(frame_idx * z_spacing))
        ...
        for i, point in enumerate(contour):
            if isinstance(point, np.ndarray) and point.size == 2:
                x, y = point[0], point[1]
                z = frame_idx * z_spacing
                ET.SubElement(control_points_elem, "point", id=str(i), x=str(x), y=str(y), z=str(z))
                ET.SubElement(contour_points_elem, "point", id=str(i), x=str(x), y=str(y), z=str(z))
            else:
...
def plot_contours_3d(contours, z_spacing):
    fig = plt.figure()
    ...
    for idx, (contour, frame_idx) in enumerate(contours):
        xs = contour[:, 0]
        ys = contour[:, 1]
        zs = np.full(xs.shape, frame_idx * z_spacing)  # z coordinate based on frame index
        ax.plot(xs, ys, zs)
...
def main():
    ...
    number_of_frames = 4 #total_frames  # Use the actual number of frames
    ...
    outer_contours_list = []
    inner_contours_list = []
    z_spacing = 100.0  # Define the spacing between frames in the z-direction
    ...
    save_contour_as_ctgr(outer_contours_list, os.path.join(contours_dir_ctgr, f'contours_outer.ctgr'), z_spacing)
    
    # Save inner contours in .ctgr format
    save_contour_as_ctgr(inner_contours_list, os.path.join(contours_dir_ctgr, f'contours_inner.ctgr'), z_spacing)
    
    plot_contours_3d(contours_list, z_spacing)
